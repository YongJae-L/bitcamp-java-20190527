...
public class MybatisDaoFactory {
  ...
  public <T> T createDao(Class<T> clazz) { //어떤 타입의 객체가 만들어 질지 모르니까 <T> 제네릭 사용!  !=! T라는 인터페이스의 정보가 넘어온다.
    return (T) Proxy.newProxyInstance(clazz.getClassLoader(), new Class[] {clazz},
        (Object proxy, Method method, Object[] args) -> {
            // InvocationHandler 구현체의 람다(lambda) 메서드
          // 자동으로 생성된 DAO 구현체에 대해 메서드를 호출하면 최종적으로 이 메서드가 호출된다.
          String interfaceName = clazz.getSimpleName();
          String methodName = method.getName(); //호출된 메서드 이름 알아내기
          String sqlId = interfaceName + "." + methodName; //이름을 결합해서 인터페이스 아이디를 만든다.
          
          Class<?> retType = method.getReturnType();
          
          try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            // select/insert/update/delete를 구분하여 메서드를 호출
            // 어떻게? return 타입을 보고 결정한다.
            // => List라면 selectList()를 호출하고,
            // => 그냥 일반 객체라면 selectOne()을 호출하고,
            // => int 라면 insert()/update()/detele()를 호출한다.
