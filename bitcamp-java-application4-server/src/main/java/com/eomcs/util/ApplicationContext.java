...
//    // => 찾은 클래스의 인스턴스를 생성한다.
  ...
  private boolean isCommand(Class<?> clazz) {
    ...
    for(Class<?> c : interfaces) {
      if(c == Command.class) { // 클래스정보는 한 개만 만들어지기 때문에 equal이 필요없다. == 주소비교
        return true;
      }
    }
    return false;
  ...
  private void createCommand() {
    for (Class<?> clazz : classes) {
      ...
      try {
        ...
        objPool.put(beanName, command);////////////////////////////////////
        //objPool.put(clazz.getName(), defaultConstructor.newInstance());
        continue;
      } catch (Exception e) {
      }
      // 다른 생성자 꺼내기
