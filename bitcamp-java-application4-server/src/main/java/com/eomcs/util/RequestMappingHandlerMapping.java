...
public class RequestMappingHandlerMapping {
  ...
  public static class RequestHandler {
    ...
    private Object getPojoValue(
      ...
      Object pojo = paramType.getConstructor().newInstance(); //빈객체 생성

      // => 세터 메서드를 추출한다.
      Set<Method> methods = getMethods(paramType, withPrefix("set"));
      log.debug(String.format("%s 의 세터 메서드:" , paramType.getName()));

      // 클라이언트가 보낸 데이터 중에서 세터 메서드에 넘겨줄 요청 데이터가 있다면 호출한다.
      for(Method m : methods) {
        // 메서드 이름에서 프로퍼티 명을 추출한다.
        String propName = m.getName().substring(3,4).toLowerCase()+m.getName().substring(4); // setCreatedDate() => createdDate
        // 세터를 호춣할 때 넘겨 줄 값을 담을 변수를 준비.
        Object value = null;
        ...
        // 클라이언트가 보낸 값 중에서 파라미터 이름과 일치하는 값
