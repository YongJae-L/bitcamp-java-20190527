...
public class App {
  ...
  private static void loadLessonData() {
    ...
    try {
      ...
      //파일리더는 Readable 인터페이스를 위 위클래스에서 구현함 그래서 아래서 in 사용가능
      scan = new Scanner(in);
      while(scan.hasNextLine()) {
        // 파일에서 한 줄 읽는다.
        String line = scan.nextLine();

        // 문자열을 콤마로 분리 한다. 분리된 데이터는 배열에 담겨 리턴된다.
      ...
      try {
        ...
        //아무것도 할 필요 없다. close() 하다가 오류가 발생하면 무시한다.
      }
      try {
        in.close();
      } catch (Exception e) {
