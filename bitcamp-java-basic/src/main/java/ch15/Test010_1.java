...
public class Test010_1 {
  public static void main(String[] args) {
    ...
    //         key 객체의 hashCode()를 호출하여 그 리턴 값을 사용하여 위치를 나타낸다.
    // => map.put(key, value);
    //
...
// => 그렇게 리턴 받은 값(해시코드)를 사용하여 Student 객체를 저장할 위치를 계산한다.
...
//    그리고 그 객체를 넘겨주는 것이다.
    ...
    // 인스턴스 필드의 값이 같은지 따지지 않고 인스턴스마다 고유의 해시 값을 리턴한다.
    // 또한 equals()는 인스턴스 필드의 값이 같은지 비교하지 않고 같은 인스턴스인지만 따진다.
    //
    // 그러나 Integer 클래스 동의 wrapper 클래스와 String 클래스는
    // Object 클래스에서 상속 받은 hashCode()와 equlas()를 오버라이딩 했기 때문에
    // 인스턴스가 다르더라도 인스턴스 필드의 값이 같으면 
    // hashCode()의 리턴 값이 같고 equals()가 true를 리턴한다.
    //
    // 그래서 wrapper 클래스와 String 클래스는 HashMap/Hashtable의
    // key로 사용할 수 있는 것이다.
    // 실무에서도 String이나 primitive type(결국 wrapper 객체로 전혼되기 때문)을 key로 자주 사용한다.
    //
